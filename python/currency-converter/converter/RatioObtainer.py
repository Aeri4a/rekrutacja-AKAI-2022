...
class RatioObtainer:
    ...
    def was_ratio_saved_today(self):
        ...
        with open("ratios.json", "r") as fc:
            data = json.load(fc)

        for d in data:
            if d['base_currency'] == self.base and d['target_currency'] == self.target and d['date_fetched'] == today:
                return True
    ...
    def save_ratio(self, ratio):
        ...
        with open("ratios.json", "r") as fr:
            data = json.load(fr)

        for d in data:
            if d['base_currency'] == self.base and d['target_currency'] == self.target:
                d['ratio'] = ratio
                d['date_fetched'] = today
                change = True
        ...
        with open("ratios.json", "w", encoding="utf-8") as fs:
